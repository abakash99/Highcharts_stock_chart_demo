import React, { useState, useEffect, useRef } from "react";

// import Highcharts from "highcharts";
import HighchartsStock from "highcharts/modules/stock";
import HighchartsReact from "highcharts-react-official";
import Highcharts from "highcharts/highstock";
import axios from "axios";
import { Button } from "@material-ui/core";
import { AdapterMoment } from "@mui/x-date-pickers/AdapterMoment";
import { LocalizationProvider } from "@mui/x-date-pickers/LocalizationProvider";
import { DatePicker } from "@mui/x-date-pickers";
import TextField from "@mui/material/TextField";
import moment from "moment";
import HC_more from "highcharts/highcharts-more"; //module for bubble chart
HC_more(Highcharts); //init module

Highcharts.Chart.prototype.showResetZoom = function () {}; // to hide the default zoom button for buble chart

HighchartsStock(Highcharts);

export const App = () => {
  /**
   * sankey chart options
   */
  // const data = {
  //   title: {
  //     text: "",
  //   },
  //   series: [
  //     {
  //       keys: ["from", "to", "weight"],
  //       data: [
  //         ["Brazil", "Portugal", 5],
  //         ["Brazil", "France", 1],
  //         ["Brazil", "Spain", 1],
  //         ["Brazil", "England", 1],
  //         ["Canada", "Portugal", 1],
  //         ["Canada", "France", 5],
  //         ["Canada", "England", 1],
  //         ["Mexico", "Portugal", 1],
  //         ["Mexico", "France", 1],
  //         ["Mexico", "Spain", 5],
  //         ["Mexico", "England", 1],
  //         ["USA", "Portugal", 1],
  //         ["USA", "France", 1],
  //         ["USA", "Spain", 1],
  //         ["USA", "England", 5],
  //         ["Portugal", "Angola", 2],
  //         ["Portugal", "Senegal", 1],
  //         ["Portugal", "Morocco", 1],
  //         ["Portugal", "South Africa", 3],
  //         ["France", "Angola", 1],
  //         ["France", "Senegal", 3],
  //         ["France", "Mali", 3],
  //         ["France", "Morocco", 3],
  //         ["France", "South Africa", 1],
  //         ["Spain", "Senegal", 1],
  //         ["Spain", "Morocco", 3],
  //         ["Spain", "South Africa", 1],
  //         ["England", "Angola", 1],
  //         ["England", "Senegal", 1],
  //         ["England", "Morocco", 2],
  //         ["England", "South Africa", 7],
  //         ["South Africa", "China", 5],
  //         ["South Africa", "India", 1],
  //         ["South Africa", "Japan", 3],
  //         ["Angola", "China", 5],
  //         ["Angola", "India", 1],
  //         ["Angola", "Japan", 3],
  //         ["Senegal", "China", 5],
  //         ["Senegal", "India", 1],
  //         ["Senegal", "Japan", 3],
  //         ["Mali", "China", 5],
  //         ["Mali", "India", 1],
  //         ["Mali", "Japan", 3],
  //         ["Morocco", "China", 5],
  //         ["Morocco", "India", 1],
  //         ["Morocco", "Japan", 3],
  //       ],
  //       type: "sankey",
  //       name: "Sankey demo series",
  //     },
  //   ],
  // };

  const singleLineOptions = {
    title: {
      text: "Single line stock chart",
    },
    yAxis: {
      opposite: false,
    },
    series: [
      {
        data: [
          [1586871000000, 71.76],
          [1586957400000, 71.11],
          [1587043800000, 71.67],
          [1587130200000, 70.7],
          [1587389400000, 69.23],
          [1587475800000, 67.09],
          [1587562200000, 69.03],
          [1587648600000, 68.76],
          [1587735000000, 70.74],
          [1587994200000, 70.79],
          [1588080600000, 69.64],
          [1588167000000, 71.93],
          [1588253400000, 73.45],
          [1588339800000, 72.27],
          [1588599000000, 73.29],
          [1588685400000, 74.39],
          [1588771800000, 75.16],
          [1588858200000, 75.93],
          [1588944600000, 77.53],
          [1589203800000, 78.75],
          [1589290200000, 77.85],
          [1589376600000, 76.91],
          [1589463000000, 77.39],
          [1589549400000, 76.93],
          [1589808600000, 78.74],
          [1589895000000, 78.29],
          [1589981400000, 79.81],
          [1590067800000, 79.21],
          [1590154200000, 79.72],
          [1590499800000, 79.18],
          [1590586200000, 79.53],
          [1590672600000, 79.56],
          [1590759000000, 79.49],
          [1591018200000, 80.46],
          [1591104600000, 80.83],
          [1591191000000, 81.28],
          [1591277400000, 80.58],
          [1591363800000, 82.88],
          [1591623000000, 83.36],
          [1591709400000, 86],
          [1591795800000, 88.21],
          [1591882200000, 83.97],
          [1591968600000, 84.7],
          [1592227800000, 85.75],
          [1592314200000, 88.02],
          [1592400600000, 87.9],
          [1592487000000, 87.93],
          [1592573400000, 87.43],
          [1592832600000, 89.72],
          [1592919000000, 91.63],
          [1593005400000, 90.01],
          [1593091800000, 91.21],
          [1593178200000, 88.41],
          [1593437400000, 90.44],
          [1593523800000, 91.2],
          [1593610200000, 91.03],
          [1593696600000, 91.03],
          [1594042200000, 93.46],
          [1594128600000, 93.17],
          [1594215000000, 95.34],
          [1594301400000, 95.75],
          [1594387800000, 95.92],
          [1594647000000, 95.48],
          [1594733400000, 97.06],
          [1594819800000, 97.72],
          [1594906200000, 96.52],
          [1594992600000, 96.33],
          [1595251800000, 98.36],
          [1595338200000, 97],
          [1595424600000, 97.27],
          [1595511000000, 92.85],
          [1595597400000, 92.61],
          [1595856600000, 94.81],
          [1595943000000, 93.25],
          [1596029400000, 95.04],
          [1596115800000, 96.19],
          [1596202200000, 106.26],
          [1596461400000, 108.94],
          [1596547800000, 109.67],
          [1596634200000, 110.06],
          [1596720600000, 113.9],
          [1596807000000, 111.11],
          [1597066200000, 112.73],
          [1597152600000, 109.38],
          [1597239000000, 113.01],
          [1597325400000, 115.01],
          [1597411800000, 114.91],
          [1597671000000, 114.61],
          [1597757400000, 115.56],
          [1597843800000, 115.71],
          [1597930200000, 118.28],
          [1598016600000, 124.37],
          [1598275800000, 125.86],
          [1598362200000, 124.82],
          [1598448600000, 126.52],
          [1598535000000, 125.01],
          [1598621400000, 124.81],
          [1598880600000, 129.04],
          [1598967000000, 134.18],
          [1599053400000, 131.4],
          [1599139800000, 120.88],
          [1599226200000, 120.96],
          [1599571800000, 112.82],
          [1599658200000, 117.32],
          [1599744600000, 113.49],
          [1599831000000, 112],
          [1600090200000, 115.36],
          [1600176600000, 115.54],
          [1600263000000, 112.13],
          [1600349400000, 110.34],
          [1600435800000, 106.84],
          [1600695000000, 110.08],
          [1600781400000, 111.81],
          [1600867800000, 107.12],
          [1600954200000, 108.22],
          [1601040600000, 112.28],
          [1601299800000, 114.96],
          [1601386200000, 114.09],
          [1601472600000, 115.81],
          [1601559000000, 116.79],
          [1601645400000, 113.02],
          [1601904600000, 116.5],
          [1601991000000, 113.16],
          [1602077400000, 115.08],
          [1602163800000, 114.97],
          [1602250200000, 116.97],
          [1602509400000, 124.4],
          [1602595800000, 121.1],
          [1602682200000, 121.19],
          [1602768600000, 120.71],
          [1602855000000, 119.02],
          [1603114200000, 115.98],
          [1603200600000, 117.51],
          [1603287000000, 116.87],
          [1603373400000, 115.75],
          [1603459800000, 115.04],
          [1603719000000, 115.05],
          [1603805400000, 116.6],
          [1603891800000, 111.2],
          [1603978200000, 115.32],
          [1604064600000, 108.86],
          [1604327400000, 108.77],
          [1604413800000, 110.44],
          [1604500200000, 114.95],
          [1604586600000, 119.03],
          [1604673000000, 118.69],
          [1604932200000, 116.32],
          [1605018600000, 115.97],
          [1605105000000, 119.49],
          [1605191400000, 119.21],
          [1605277800000, 119.26],
          [1605537000000, 120.3],
          [1605623400000, 119.39],
          [1605709800000, 118.03],
          [1605796200000, 118.64],
          [1605882600000, 117.34],
          [1606141800000, 113.85],
          [1606228200000, 115.17],
          [1606314600000, 116.03],
          [1606487400000, 116.59],
          [1606746600000, 119.05],
          [1606833000000, 122.72],
          [1606919400000, 123.08],
          [1607005800000, 122.94],
          [1607092200000, 122.25],
          [1607351400000, 123.75],
          [1607437800000, 124.38],
          [1607524200000, 121.78],
          [1607610600000, 123.24],
          [1607697000000, 122.41],
          [1607956200000, 121.78],
          [1608042600000, 127.88],
          [1608129000000, 127.81],
          [1608215400000, 128.7],
          [1608301800000, 126.66],
          [1608561000000, 128.23],
          [1608647400000, 131.88],
          [1608733800000, 130.96],
          [1608820200000, 131.97],
          [1609165800000, 136.69],
          [1609252200000, 134.87],
          [1609338600000, 133.72],
          [1609425000000, 132.69],
          [1609770600000, 129.41],
          [1609857000000, 131.01],
          [1609943400000, 126.6],
          [1610029800000, 130.92],
          [1610116200000, 132.05],
          [1610375400000, 128.98],
          [1610461800000, 128.8],
          [1610548200000, 130.89],
          [1610634600000, 128.91],
          [1610721000000, 127.14],
          [1611066600000, 127.83],
          [1611153000000, 132.03],
          [1611239400000, 136.87],
          [1611325800000, 139.07],
          [1611585000000, 142.92],
          [1611671400000, 143.16],
          [1611757800000, 142.06],
          [1611844200000, 137.09],
          [1611930600000, 131.96],
          [1612189800000, 134.14],
          [1612276200000, 134.99],
          [1612362600000, 133.94],
          [1612449000000, 137.39],
          [1612535400000, 136.76],
          [1612794600000, 136.91],
          [1612881000000, 136.01],
          [1612967400000, 135.39],
          [1613053800000, 135.13],
          [1613140200000, 135.37],
          [1613485800000, 133.19],
          [1613572200000, 130.84],
          [1613658600000, 129.71],
          [1613745000000, 129.87],
          [1614004200000, 126],
          [1614090600000, 125.86],
          [1614177000000, 125.35],
          [1614263400000, 120.99],
          [1614349800000, 121.26],
          [1614609000000, 127.79],
          [1614695400000, 125.12],
          [1614781800000, 122.06],
          [1614868200000, 120.13],
          [1614954600000, 121.42],
          [1615213800000, 116.36],
          [1615300200000, 121.09],
          [1615386600000, 119.98],
          [1615473000000, 121.96],
          [1615559400000, 121.03],
          [1615815000000, 123.99],
          [1615901400000, 125.57],
          [1615987800000, 124.76],
          [1616074200000, 120.53],
          [1616160600000, 119.99],
          [1616419800000, 123.39],
          [1616506200000, 122.54],
          [1616592600000, 120.09],
          [1616679000000, 120.59],
          [1616765400000, 121.21],
          [1617024600000, 121.39],
          [1617111000000, 119.9],
          [1617197400000, 122.15],
          [1617283800000, 123],
          [1617629400000, 125.9],
          [1617715800000, 126.21],
          [1617802200000, 127.9],
          [1617888600000, 130.36],
          [1617975000000, 133],
          [1618234200000, 131.24],
          [1618320600000, 134.43],
          [1618407000000, 132.03],
          [1618493400000, 134.5],
          [1618579800000, 134.16],
          [1618839000000, 134.84],
          [1618925400000, 133.11],
          [1619011800000, 133.5],
          [1619098200000, 131.94],
          [1619184600000, 134.32],
          [1619443800000, 134.72],
          [1619530200000, 134.39],
          [1619616600000, 133.58],
          [1619703000000, 133.48],
          [1619789400000, 131.46],
          [1620048600000, 132.54],
          [1620135000000, 127.85],
          [1620221400000, 128.1],
          [1620307800000, 129.74],
          [1620394200000, 130.21],
          [1620653400000, 126.85],
          [1620739800000, 125.91],
          [1620826200000, 122.77],
          [1620912600000, 124.97],
          [1620999000000, 127.45],
          [1621258200000, 126.27],
          [1621344600000, 124.85],
          [1621431000000, 124.69],
          [1621517400000, 127.31],
          [1621603800000, 125.43],
          [1621863000000, 127.1],
          [1621949400000, 126.9],
          [1622035800000, 126.85],
          [1622122200000, 125.28],
          [1622208600000, 124.61],
          [1622554200000, 124.28],
          [1622640600000, 125.06],
          [1622727000000, 123.54],
          [1622813400000, 125.89],
          [1623072600000, 125.9],
          [1623159000000, 126.74],
          [1623245400000, 127.13],
          [1623331800000, 126.11],
          [1623418200000, 127.35],
          [1623677400000, 130.48],
          [1623763800000, 129.64],
          [1623850200000, 130.15],
          [1623936600000, 131.79],
          [1624023000000, 130.46],
          [1624282200000, 132.3],
          [1624368600000, 133.98],
          [1624455000000, 133.7],
          [1624541400000, 133.41],
          [1624627800000, 133.11],
          [1624887000000, 134.78],
          [1624973400000, 136.33],
          [1625059800000, 136.96],
          [1625146200000, 137.27],
          [1625232600000, 139.96],
          [1625578200000, 142.02],
          [1625664600000, 144.57],
          [1625751000000, 143.24],
          [1625837400000, 145.11],
          [1626096600000, 144.5],
          [1626183000000, 145.64],
          [1626269400000, 149.15],
          [1626355800000, 148.48],
          [1626442200000, 146.39],
          [1626701400000, 142.45],
          [1626787800000, 146.15],
          [1626874200000, 145.4],
          [1626960600000, 146.8],
          [1627047000000, 148.56],
          [1627306200000, 148.99],
          [1627392600000, 146.77],
          [1627479000000, 144.98],
          [1627565400000, 145.64],
          [1627651800000, 145.86],
          [1627911000000, 145.52],
          [1627997400000, 147.36],
          [1628083800000, 146.95],
          [1628170200000, 147.06],
          [1628256600000, 146.14],
          [1628515800000, 146.09],
          [1628602200000, 145.6],
          [1628688600000, 145.86],
          [1628775000000, 148.89],
          [1628861400000, 149.1],
          [1629120600000, 151.12],
          [1629207000000, 150.19],
          [1629293400000, 146.36],
          [1629379800000, 146.7],
          [1629466200000, 148.19],
          [1629725400000, 149.71],
          [1629811800000, 149.62],
          [1629898200000, 148.36],
          [1629984600000, 147.54],
          [1630071000000, 148.6],
          [1630330200000, 153.12],
          [1630416600000, 151.83],
          [1630503000000, 152.51],
          [1630589400000, 153.65],
          [1630675800000, 154.3],
          [1631021400000, 156.69],
          [1631107800000, 155.11],
          [1631194200000, 154.07],
          [1631280600000, 148.97],
          [1631539800000, 149.55],
          [1631626200000, 148.12],
          [1631712600000, 149.03],
          [1631799000000, 148.79],
          [1631885400000, 146.06],
          [1632144600000, 142.94],
          [1632231000000, 143.43],
          [1632317400000, 145.85],
          [1632403800000, 146.83],
          [1632490200000, 146.92],
          [1632749400000, 145.37],
          [1632835800000, 141.91],
          [1632922200000, 142.83],
          [1633008600000, 141.5],
          [1633095000000, 142.65],
          [1633354200000, 139.14],
          [1633440600000, 141.11],
          [1633527000000, 142],
          [1633613400000, 143.29],
          [1633699800000, 142.9],
          [1633959000000, 142.81],
          [1634045400000, 141.51],
          [1634131800000, 140.91],
          [1634218200000, 143.76],
          [1634304600000, 144.84],
          [1634563800000, 146.55],
          [1634650200000, 148.76],
          [1634736600000, 149.26],
          [1634823000000, 149.48],
          [1634909400000, 148.69],
          [1635168600000, 148.64],
          [1635255000000, 149.32],
          [1635341400000, 148.85],
          [1635427800000, 152.57],
          [1635514200000, 149.8],
          [1635773400000, 148.96],
          [1635859800000, 150.02],
          [1635946200000, 151.49],
          [1636032600000, 150.96],
          [1636119000000, 151.28],
          [1636381800000, 150.44],
          [1636468200000, 150.81],
          [1636554600000, 147.92],
          [1636641000000, 147.87],
          [1636727400000, 149.99],
          [1636986600000, 150],
          [1637073000000, 151],
          [1637159400000, 153.49],
          [1637245800000, 157.87],
          [1637332200000, 160.55],
          [1637591400000, 161.02],
          [1637677800000, 161.41],
          [1637764200000, 161.94],
          [1637937000000, 156.81],
          [1638196200000, 160.24],
          [1638282600000, 165.3],
          [1638369000000, 164.77],
          [1638455400000, 163.76],
          [1638541800000, 161.84],
          [1638801000000, 165.32],
          [1638887400000, 171.18],
          [1638973800000, 175.08],
          [1639060200000, 174.56],
          [1639146600000, 179.45],
          [1639405800000, 175.74],
          [1639492200000, 174.33],
          [1639578600000, 179.3],
          [1639665000000, 172.26],
          [1639751400000, 171.14],
          [1640010600000, 169.75],
          [1640097000000, 172.99],
          [1640183400000, 175.64],
          [1640269800000, 176.28],
          [1640615400000, 180.33],
          [1640701800000, 179.29],
          [1640788200000, 179.38],
          [1640874600000, 178.2],
          [1640961000000, 177.57],
          [1641220200000, 182.01],
          [1641306600000, 179.7],
          [1641393000000, 174.92],
          [1641479400000, 172],
          [1641565800000, 172.17],
          [1641825000000, 172.19],
          [1641911400000, 175.08],
          [1641997800000, 175.53],
          [1642084200000, 172.19],
          [1642170600000, 173.07],
          [1642516200000, 169.8],
          [1642602600000, 166.23],
          [1642689000000, 164.51],
          [1642775400000, 162.41],
          [1643034600000, 161.62],
          [1643121000000, 159.78],
          [1643207400000, 159.69],
          [1643293800000, 159.22],
          [1643380200000, 170.33],
          [1643639400000, 174.78],
          [1643725800000, 174.61],
          [1643812200000, 175.84],
          [1643898600000, 172.9],
          [1643985000000, 172.39],
          [1644244200000, 171.66],
          [1644330600000, 174.83],
          [1644417000000, 176.28],
          [1644503400000, 172.12],
          [1644589800000, 168.64],
          [1644849000000, 168.88],
          [1644935400000, 172.79],
          [1645021800000, 172.55],
          [1645108200000, 168.88],
          [1645194600000, 167.3],
          [1645540200000, 164.32],
          [1645626600000, 160.07],
          [1645713000000, 162.74],
          [1645799400000, 164.85],
          [1646058600000, 165.12],
          [1646145000000, 163.2],
          [1646231400000, 166.56],
          [1646317800000, 166.23],
          [1646404200000, 163.17],
          [1646663400000, 159.3],
          [1646749800000, 157.44],
          [1646836200000, 162.95],
          [1646922600000, 158.52],
          [1647009000000, 154.73],
          [1647264600000, 150.62],
          [1647351000000, 155.09],
          [1647437400000, 159.59],
          [1647523800000, 160.62],
          [1647610200000, 163.98],
          [1647869400000, 165.38],
          [1647955800000, 168.82],
          [1648042200000, 170.21],
          [1648128600000, 174.07],
          [1648215000000, 174.72],
          [1648474200000, 175.6],
          [1648560600000, 178.96],
          [1648647000000, 177.77],
          [1648733400000, 174.61],
          [1648819800000, 174.31],
          [1649079000000, 178.44],
          [1649165400000, 175.06],
          [1649251800000, 171.83],
          [1649338200000, 172.14],
          [1649424600000, 170.09],
          [1649683800000, 165.75],
          [1649770200000, 167.66],
          [1649856600000, 170.4],
        ],
      },
    ],
    rangeSelector: {
      buttons: [],
      inputEnabled: false,
    },
  };

  const areaOptions = {
    title: {
      text: "area stock chart",
    },
    chart: {
      type: "area",
    },
    yAxis: {
      opposite: false,
    },
    series: [
      {
        data: [
          29.9, 71.5, 106.4, 129.2, 144.0, 176.0, 135.6, 148.5, 216.4, 194.1,
          95.6, 54.4,
        ],
        // fillColor:
        //   "linear-gradient(180deg, rgba(4, 139, 208, 0.6) 0%, rgba(4, 139, 208, 0) 100%);",
      },
    ],
    rangeSelector: {
      buttons: [],
      inputEnabled: false,
    },
  };
  const areaOptionsWithNoDateRange = {
    title: {
      text: "area stock chart without date range graph",
    },
    chart: {
      type: "area",
    },
    yAxis: {
      opposite: false,
    },
    series: [
      {
        data: [
          29.9, 71.5, 106.4, 129.2, 144.0, 176.0, 135.6, 148.5, 216.4, 194.1,
          95.6, 54.4,
        ],
        // fillColor:
        //   "linear-gradient(180deg, rgba(4, 139, 208, 0.6) 0%, rgba(4, 139, 208, 0) 100%);",
      },
    ],
    rangeSelector: {
      buttons: [],
      inputEnabled: false,
    },
    navigator: {
      enabled: false,
    },
  };

  const extreme = useRef();
  const columnOptions = {
    title: {
      text: "My column stock chart",
    },
    chart: {
      events: {
        load: (event) => {
          extreme.current = event.target.xAxis;
          // extreme.forEach((axis) => {
          //   console.log("event is", axis.getExtremes());
          // });
        },
      },
      type: "column",
    },
    xAxis: {
      labels: {
        // to add some space between label and graph
        y: 30,
      },
      events: {
        afterSetExtremes: (params) =>
          setDateValue({
            from: moment(params.userMin),
            to: moment(params.userMax),
          }),
      },
    },
    yAxis: {
      opposite: false,
    },
    series: [
      {
        data: [
          [1586871000000, 194994800],
          [1586957400000, 131154400],
          [1587043800000, 157125200],
          [1587130200000, 215250000],
          [1587389400000, 130015200],
          [1587475800000, 180991600],
          [1587562200000, 116862400],
          [1587648600000, 124814400],
          [1587735000000, 126161200],
          [1587994200000, 117087600],
          [1588080600000, 112004800],
          [1588167000000, 137280800],
          [1588253400000, 183064000],
          [1588339800000, 240616800],
          [1588599000000, 133568000],
          [1588685400000, 147751200],
          [1588771800000, 142333600],
          [1588858200000, 115215200],
          [1588944600000, 133838400],
          [1589203800000, 145946400],
          [1589290200000, 162301200],
          [1589376600000, 200622400],
          [1589463000000, 158929200],
          [1589549400000, 166348400],
          [1589808600000, 135178400],
          [1589895000000, 101729600],
          [1589981400000, 111504800],
          [1590067800000, 102688800],
          [1590154200000, 81803200],
          [1590499800000, 125522000],
          [1590586200000, 112945200],
          [1590672600000, 133560800],
          [1590759000000, 153532400],
          [1591018200000, 80791200],
          [1591104600000, 87642800],
          [1591191000000, 104491200],
          [1591277400000, 87560400],
          [1591363800000, 137250400],
          [1591623000000, 95654400],
          [1591709400000, 147712400],
          [1591795800000, 166651600],
          [1591882200000, 201662400],
          [1591968600000, 200146000],
          [1592227800000, 138808800],
          [1592314200000, 165428800],
          [1592400600000, 114406400],
          [1592487000000, 96820400],
          [1592573400000, 264476000],
          [1592832600000, 135445200],
          [1592919000000, 212155600],
          [1593005400000, 192623200],
          [1593091800000, 137522400],
          [1593178200000, 205256800],
          [1593437400000, 130646000],
          [1593523800000, 140223200],
          [1593610200000, 110737200],
          [1593696600000, 114041600],
          [1594042200000, 118655600],
          [1594128600000, 112424400],
          [1594215000000, 117092000],
          [1594301400000, 125642800],
          [1594387800000, 90257200],
          [1594647000000, 191649200],
          [1594733400000, 170989200],
          [1594819800000, 153198000],
          [1594906200000, 110577600],
          [1594992600000, 92186800],
          [1595251800000, 90318000],
          [1595338200000, 103433200],
          [1595424600000, 89001600],
          [1595511000000, 197004400],
          [1595597400000, 185438800],
          [1595856600000, 121214000],
          [1595943000000, 103625600],
          [1596029400000, 90329200],
          [1596115800000, 158130000],
          [1596202200000, 374336800],
          [1596461400000, 308151200],
          [1596547800000, 173071600],
          [1596634200000, 121776800],
          [1596720600000, 202428800],
          [1596807000000, 198045600],
          [1597066200000, 212403600],
          [1597152600000, 187902400],
          [1597239000000, 165598000],
          [1597325400000, 210082000],
          [1597411800000, 165565200],
          [1597671000000, 119561600],
          [1597757400000, 105633600],
          [1597843800000, 145538000],
          [1597930200000, 126907200],
          [1598016600000, 338054800],
          [1598275800000, 345937600],
          [1598362200000, 211495600],
          [1598448600000, 163022400],
          [1598535000000, 155552400],
          [1598621400000, 187630000],
          [1598880600000, 225702700],
          [1598967000000, 151948100],
          [1599053400000, 200119000],
          [1599139800000, 257599600],
          [1599226200000, 332607200],
          [1599571800000, 231366600],
          [1599658200000, 176940500],
          [1599744600000, 182274400],
          [1599831000000, 180860300],
          [1600090200000, 140150100],
          [1600176600000, 184642000],
          [1600263000000, 154679000],
          [1600349400000, 178011000],
          [1600435800000, 287104900],
          [1600695000000, 195713800],
          [1600781400000, 183055400],
          [1600867800000, 150718700],
          [1600954200000, 167743300],
          [1601040600000, 149981400],
          [1601299800000, 137672400],
          [1601386200000, 99382200],
          [1601472600000, 142675200],
          [1601559000000, 116120400],
          [1601645400000, 144712000],
          [1601904600000, 106243800],
          [1601991000000, 161498200],
          [1602077400000, 96849000],
          [1602163800000, 83477200],
          [1602250200000, 100506900],
          [1602509400000, 240226800],
          [1602595800000, 262330500],
          [1602682200000, 150712000],
          [1602768600000, 112559200],
          [1602855000000, 115393800],
          [1603114200000, 120639300],
          [1603200600000, 124423700],
          [1603287000000, 89946000],
          [1603373400000, 101988000],
          [1603459800000, 82572600],
          [1603719000000, 111850700],
          [1603805400000, 92276800],
          [1603891800000, 143937800],
          [1603978200000, 146129200],
          [1604064600000, 190272600],
        ],
      },
    ],
    rangeSelector: {
      buttons: [],
      inputEnabled: false,
    },
    // navigator: {
    //   // enabled: false,
    // },
  };

  /**
  compare series stock chart
  *  */
  const [tempOptions, setTempOptions] = useState([]);
  let seriesOptions = [],
    seriesCounter = 0,
    names = ["MSFT", "AAPL", "GOOG"];
  let i = 0;
  function success(data) {
    seriesOptions[i] = {
      name: names[i],
      data: [...data],
    };
    i += 1;

    seriesCounter += 1;

    if (seriesCounter === names.length) {
      setTempOptions(seriesOptions);
    }
  }

  useEffect(() => {
    const msft =
      "https://cdn.jsdelivr.net/gh/highcharts/highcharts@v7.0.0/samples/data/msft-c.json";
    const apl =
      "https://cdn.jsdelivr.net/gh/highcharts/highcharts@v7.0.0/samples/data/aapl-c.json";
    const gogl =
      "https://cdn.jsdelivr.net/gh/highcharts/highcharts@v7.0.0/samples/data/goog-c.json";

    [msft, apl, gogl].forEach(async (item) => {
      axios
        .get(item)
        .then((res) => success(res.data))
        .catch();
    });
  }, []);

  const [dateValue, setDateValue] = useState({
    from: null,
    to: null,
  });

  // bubble chart options

  const chartComponent = useRef(null);
  const [isZoomed, setIsZoomed] = useState(false);

  const bubbleChartOptions = {
    chart: {
      type: "bubble",
      plotBorderWidth: 1,
      zoomType: "xy",
      events: {
        selection: function (e) {
          if (e.resetSelection) {
            setIsZoomed(false);
          } else {
            setIsZoomed(true);
          }
        },
      },
    },

    legend: {
      enabled: false,
    },

    title: {
      text: "Sugar and fat intake per country",
    },

    subtitle: {
      text: 'Source: <a href="http://www.euromonitor.com/">Euromonitor</a> and <a href="https://data.oecd.org/">OECD</a>',
    },

    accessibility: {
      point: {
        valueDescriptionFormat:
          "{index}. {point.name}, fat: {point.x}g, sugar: {point.y}g, obesity: {point.z}%.",
      },
    },

    xAxis: {
      gridLineWidth: 1,
      title: {
        text: "Daily fat intake",
      },
      labels: {
        format: "{value} gr",
      },
      plotLines: [
        {
          color: "black",
          dashStyle: "dot",
          width: 2,
          value: 65,
          label: {
            rotation: 0,
            y: 15,
            style: {
              fontStyle: "italic",
            },
            text: "Safe fat intake 65g/day",
          },
          zIndex: 3,
        },
      ],
      accessibility: {
        rangeDescription: "Range: 60 to 100 grams.",
      },
    },

    yAxis: {
      startOnTick: false,
      endOnTick: false,
      title: {
        text: "Daily sugar intake",
      },
      labels: {
        format: "{value} gr",
      },
      maxPadding: 0.2,
      plotLines: [
        {
          color: "black",
          dashStyle: "dot",
          width: 2,
          value: 50,
          label: {
            align: "right",
            style: {
              fontStyle: "italic",
            },
            text: "Safe sugar intake 50g/day",
            x: -10,
          },
          zIndex: 3,
        },
      ],
      accessibility: {
        rangeDescription: "Range: 0 to 160 grams.",
      },
    },

    tooltip: {
      useHTML: true,
      headerFormat: "<table>",
      pointFormat:
        '<tr><th colspan="2"><h3>{point.country}</h3></th></tr>' +
        "<tr><th>Fat intake:</th><td>{point.x}g</td></tr>" +
        "<tr><th>Sugar intake:</th><td>{point.y}g</td></tr>" +
        "<tr><th>Obesity (adults):</th><td>{point.z}%</td></tr>",
      footerFormat: "</table>",
      followPointer: true,
    },

    plotOptions: {
      series: {
        dataLabels: {
          enabled: true,
          format: "{point.name}",
        },
      },
    },

    series: [
      {
        data: [
          { x: 95, y: 95, z: 13.8, name: "BE", country: "Belgium" },
          { x: 86.5, y: 102.9, z: 14.7, name: "DE", country: "Germany" },
          { x: 80.8, y: 91.5, z: 15.8, name: "FI", country: "Finland" },
          { x: 80.4, y: 102.5, z: 12, name: "NL", country: "Netherlands" },
          { x: 80.3, y: 86.1, z: 11.8, name: "SE", country: "Sweden" },
          { x: 78.4, y: 70.1, z: 16.6, name: "ES", country: "Spain" },
          { x: 74.2, y: 68.5, z: 14.5, name: "FR", country: "France" },
          { x: 73.5, y: 83.1, z: 10, name: "NO", country: "Norway" },
          { x: 71, y: 93.2, z: 24.7, name: "UK", country: "United Kingdom" },
          { x: 69.2, y: 57.6, z: 10.4, name: "IT", country: "Italy" },
          { x: 68.6, y: 20, z: 16, name: "RU", country: "Russia" },
          { x: 65.5, y: 126.4, z: 35.3, name: "US", country: "United States" },
          { x: 65.4, y: 50.8, z: 28.5, name: "HU", country: "Hungary" },
          { x: 63.4, y: 51.8, z: 15.4, name: "PT", country: "Portugal" },
          { x: 64, y: 82.9, z: 31.3, name: "NZ", country: "New Zealand" },
        ],
      },
    ],
  };
  const resetZoom = () => {
    if (chartComponent && chartComponent.current) {
      chartComponent.current.chart.zoomOut();
    }
  };
  return (
    <div className="App">
      {/* sankey chart */}
      {/* <HighchartsReact
        highcharts={Highcharts}
        options={data}
        // constructorType="sankyChart"
      /> */}
      {/* single stock chart */}
      <div style={{ margin: "10px 0", border: "1px solid red" }}>
        <HighchartsReact
          highcharts={Highcharts}
          constructorType={"stockChart"}
          options={singleLineOptions}
        />
      </div>
      {/* compare multi stock chart */}
      <div style={{ margin: "10px 0", border: "1px solid red" }}>
        <HighchartsReact
          highcharts={Highcharts}
          constructorType={"stockChart"}
          options={{
            title: {
              text: "Multi stock chart",
            },
            yAxis: {
              labels: {
                formatter: function () {
                  return (this.value > 0 ? " + " : "") + this.value + "%";
                },
              },
              plotLines: [
                {
                  value: 0,
                  width: 2,
                  color: "yellow",
                },
              ],
              opposite: false,
            },

            plotOptions: {
              series: {
                compare: "percent",
                showInNavigator: true,
              },
            },
            tooltip: {
              // pointFormat:
              //   '<span style="color:{series.color}">{series.name}</span>: <b>{point.y}</b> ({point.change}%)<br/>',
              valueDecimals: 3,
              split: true,
            },
            series: tempOptions,
            rangeSelector: {
              buttons: [],
              inputEnabled: false,
            },
          }}
        />
      </div>

      {/* area chart */}
      <div style={{ margin: "10px 0", border: "1px solid red" }}>
        <HighchartsReact
          highcharts={Highcharts}
          constructorType={"stockChart"}
          options={areaOptions}
        />
      </div>
      {/* area chart without date range */}
      <div style={{ margin: "10px 0", border: "1px solid red" }}>
        <HighchartsReact
          highcharts={Highcharts}
          constructorType={"stockChart"}
          options={areaOptionsWithNoDateRange}
        />
      </div>

      {/* column chart  */}
      <div style={{ margin: "10px 0", border: "1px solid red" }}>
        <div>
          <LocalizationProvider dateAdapter={AdapterMoment}>
            <DatePicker
              label="from date"
              value={dateValue.from}
              onChange={(newValue) => {
                setDateValue((prev) => ({
                  ...prev,
                  from: newValue,
                }));
                extreme.current.forEach((axis) => {
                  axis.setExtremes(
                    new Date(newValue).getTime(),
                    new Date(dateValue.to).getTime()
                  );
                });
              }}
              renderInput={(params) => <TextField {...params} />}
            />
          </LocalizationProvider>
          <LocalizationProvider dateAdapter={AdapterMoment}>
            <DatePicker
              label="To Date"
              value={dateValue.to}
              onChange={(newValue) => {
                setDateValue((prev) => ({
                  ...prev,
                  to: newValue,
                }));
                extreme.current.forEach((axis) => {
                  axis.setExtremes(
                    new Date(dateValue.from).getTime(),
                    new Date(newValue).getTime()
                  );
                });
              }}
              renderInput={(params) => <TextField {...params} />}
            />
          </LocalizationProvider>
        </div>
        <HighchartsReact
          highcharts={Highcharts}
          constructorType={"stockChart"}
          options={columnOptions}
        />
      </div>
      <div style={{ margin: "10px 0", border: "1px solid red" }}>
        {isZoomed && (
          <Button
            // style={{ position: "absolute", top: 50, right: 10 }}
            onClick={resetZoom}
            color="primary"
          >
            Reset zoom custom
          </Button>
        )}
        <HighchartsReact
          ref={chartComponent}
          highcharts={Highcharts}
          options={bubbleChartOptions}
        />
      </div>
    </div>
  );
};
